 ...
 * Project archive.
 *
 * Every screenshot in `assets/img/work/` was captured from the real thing:
 * the client sites and the Next.js build were served locally from their own
 * source and photographed at 3200x2000, the Streamlit tools were run locally
 * and photographed the same way. Nothing here is a mockup.
 *
 * Fields
 *   slug        url of the case study page (work/<slug>.html)
 *   group       'cyber' | 'client'
 *   category    label shown on cards and used by the archive filter
 *   featured    appears in the Selected Work grid on the home page
 *   confidential note shown instead of a live link
 */
...
export const projects = [
   ...
   * ------------------------------------------------------------------ */
  {
    ...
    gallery: [
      { src: 'detalenc', alt: 'The DataLens upload screen with the module sidebar', caption: 'The working surface: every analysis module one click away, and an ingest that takes fifty files at a time.', ...D },
    ],
    links: [
...

  {
    ...
    gallery: [],
    links: [
      { label: 'Source', href: 'https://github.com/CHIRAG-SUTHAR-GIT/ACCOUNT-SUMMARY-CONTROL-ROOM', kind: 'code' },
    ],
...

  {
    ...
    problem:
      'The same three summaries were being rebuilt by hand every reporting cycle, from files whose columns never quite matched last time’s.',
    approach:
...

  {
    ...
    gallery: [
      { src: 'wallet', alt: 'The analysis type selector and dual upload panels', caption: 'Seven analysis modes, from withdrawal history to a custom sheet-and-column pick.', ...D },
    ],
...

  {
    ...
    accent: '#d8b4fe',
    summary:
      'Of all the client work so far, the one I am most satisfied with. Brand and portfolio site, designed and developed start to finish.',
    metrics: [
      { value: '100%', label: 'designed and built in-house' },
    ],
    ...
    problem:
      'A craft studio sells taste. The site had to be the first proof of it, which meant the design could not be borrowed from anywhere.',
    approach:
      'Brand direction, layout, motion and build were all done in-house, with the work itself given the room it needs and the interface kept out of its way.',
    features: [
    ...
    outcome: [
      'The piece of client work I would put in front of a new client first.',
    ],
  },

  {
    ...
    accent: '#38bdf8',
    summary:
      'A cinematic travel site with a scroll-driven homepage, 18 destinations and 18 packages — built with no framework, no build step and no external image requests.',
    metrics: [
    ...
    problem:
      'Travel sites live and die on their photographs, and photographs are exactly what makes them slow — especially on a phone on mobile data, which is where the bookings come from.',
    approach:
...

  {
    ...
    accent: '#2dd4bf',
    summary:
      '50+ destinations and 20 packages, searchable by destination and party size, with package-wise WhatsApp booking that carries the enquiry details into the chat.',
    metrics: [
    ...
    problem:
      'An agency’s enquiries arrive on WhatsApp, but a generic “contact us” button drops the customer into an empty chat where they have to explain themselves — and most do not bother.',
    approach:
    ...
    outcome: [
      'Enquiries arrive with context attached instead of as a blank “hi”.',
      'Fast pages on the connections that matter for this audience.',
    ],
  },

  {
    ...
    accent: '#fb923c',
    summary:
      'In daily commercial use: customer-wise delivery counts, automatic payment reminders and day-to-day accounts, with every provider’s records on the server so a lost phone is not a lost business.',
    metrics: [
    ...
    problem:
      'A tiffin provider runs on a diary: who took how many tiffins, who has paid, who is three weeks behind. The diary gets wet, the phone gets replaced, and a month of accounts disappears with it.',
    approach:
...

  {
    ...
    accent: '#4ade80',
    summary:
      'An MSME and GST-registered plastic recycling manufacturer, with a granule catalogue, product-wise enquiry, custom formulation requests and direct call and WhatsApp routes for bulk buyers.',
    metrics: [
    ...
    problem:
      'Industrial buyers are not browsing. They want to know the grade, whether the supplier is registered, and how fast they can get someone on the phone.',
    approach:
    ...
    outcome: [
      'Enquiries arrive naming the grade the buyer wants.',
      'A small manufacturer presents at the level of a much larger supplier.',
    ],
  },
];

export const featured = projects.filter((p) => p.featured);
export const byGroup = (g) => projects.filter((p) => p.group === g);
